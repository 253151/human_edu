...
public class AppMain003 extends Application {
	@Override
	public void start(Stage primaryStage) throws Exception {
		...
		button1.setText("확인1");								//text 속성 설정
		button1.setOnAction(event->Platform.exit());	//클릭 이벤트 처리
		
		Button button2 = new Button();					//Button 컨트롤 생성
		button2.setText("확인2");								//text 속성 설정
		button2.setOnAction(event->Platform.exit());	//클릭 이벤트 처리
		
		root.getChildren().add(label);						//VBox에 Label 컨트롤 추가
		root.getChildren().add(button1);					//VBox에 Button 컨트롤 추가
